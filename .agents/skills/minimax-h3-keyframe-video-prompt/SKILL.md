...
Before drafting, read `../minimax-h3-prompt/SKILL.md` and `../minimax-h3-prompt/reference/VIDEO_PROMPT_WRITING_GUIDE_base_en.md`. Treat `minimax-h3-prompt` and its bundled guide file as the canonical prompt-format specification. If this skill or its local references conflict with them, follow the canonical files.
