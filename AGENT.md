...
本项目使用项目级 Codex Skill，位置为 `.codex/skills/`。MiniMax H3 工作流依赖本项目内的 `minimax-h3-prompt` 及其下游 Skill，不依赖 `h3-prompt-writing`。

## Codex 项目级 Skill 调用约定
...
- 需要提问时，优先使用 Codex 可用的结构化用户输入；严格的是非题使用 2 个选项，其他选择题至少提供 5 个有实际差异的选项，并把推荐项放在第一位。若当前环境没有结构化提问工具，再使用简洁的自然语言提问。
...
7. 检查分辨率、时长、帧数、seed、采样步数、输出文件名和保存节点。工作流节点默认使用 `0.9 MP` 和 `720p` 分辨率，除非当前任务明确要求其他配置。确认单个工作流时长在 2–15 秒之间，并且实际时长与分镜时间序列、动作节奏和台词长度匹配；不要默认全部使用 15 秒，也不要将长视频拆出不足 2 秒的尾段。
...
工作流生成耗时可按视频时长估算：生成 `1 秒`视频大约需要 `30 秒`。因此，等待工作流完成的基础 `sleep` 时间可按“视频时长（秒）× 30”计算，并根据实例启动、排队、素材上传和结果下载情况额外增加缓冲时间。例如，`15 秒`视频的生成等待时间基础值约为 `450 秒`（`7.5 分钟`）；不要只按任务提交成功判断完成，应等待并检查实际输出文件。
