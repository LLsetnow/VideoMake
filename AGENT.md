...
单次工作流提交的最长视频时长为 15 秒，但 15 秒不是固定要求。每个工作流的实际时长应根据分镜中的时间序列、动作节奏、对白和镜头切换决定；如果一个分镜只需要 6 秒，就生成 6 秒，不要为了凑满 15 秒而延长或填充内容。
...
先确定故事目标、角色、场景、片段数量、每个片段时长、画幅比例、对白/歌词、音乐和最终剪辑顺序。每个分镜应根据其时间序列确定实际时长，单个工作流最长为 15 秒，但不要求每个工作流都生成 15 秒。优先把长故事拆成可独立生成的短镜头，避免一次工作流承担过多动作和场景变化。
...
1. 以 `workflows/` 中现有的共享模板或已验证工作流为基础，复制到当前项目目录，生成任务版本。
2. 修改输入参考内容：角色图、背景图、首帧/尾帧、参考视频和音频路径或上传对象。若视频包含人物口播，还要将对应角色目录中的音色参考文件接入工作流；例如千夏使用 `character/千夏/千夏音色6秒.mp3`。
3. 将第 5 步生成的提示词填入对应文本节点，并确保提示词中的 `<Picture N>`、`<Video N>`、`<Audio N>` 与实际接入的媒体编号一致。
4. 修改 `MiniMax H3 Audio Conditioning (T8)` 的任务类型、参考内容、音频模式及其他与当前任务有关的配置。
5. 检查分辨率、时长、帧数、seed、采样步数、输出文件名和保存节点。确认时长不超过 15 秒，并且实际时长与分镜时间序列、动作节奏和台词长度匹配；不要默认全部使用 15 秒。
...
5. 将生成结果保存或整理到当前项目目录下对应的 `output/`，并记录实例 ID、工作流文件、提示词、seed、分辨率、帧数和输出位置，便于复现；
