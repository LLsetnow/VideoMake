...
Before drafting, read `../minimax-h3-prompt/SKILL.md` and `../minimax-h3-prompt/reference/VIDEO_PROMPT_WRITING_GUIDE_ref_en.md`. Consult `../minimax-h3-prompt/reference/VIDEO_PROMPT_WRITING_GUIDE_base_en.md` for the shared shot, camera, dialogue, visible-text, and sound rules when needed. Treat `minimax-h3-prompt` and its bundled guide files as the canonical prompt-format specification. If this skill or its local references conflict with them, follow the canonical files.
