...
1. Read `../minimax-h3-prompt/SKILL.md` first.
2. Inventory the request and supplied assets.
3. Read `../minimax-h3-prompt/reference/VIDEO_PROMPT_WRITING_GUIDE_base_en.md` for T2VA, I2VA, FL2VA, or L2VA; read `../minimax-h3-prompt/reference/VIDEO_PROMPT_WRITING_GUIDE_ref_en.md` for reference generation, editing, continuation, or audio/reference work. Read both when routing is ambiguous or mixed.
...
Treat `minimax-h3-prompt` and its bundled guide files as canonical for prompt syntax. Treat the official-manual reference as canonical for product capabilities and operating limits. If a downstream skill conflicts with either authority, follow the canonical source.
